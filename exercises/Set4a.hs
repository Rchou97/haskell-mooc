...
winner scores player1 player2 | Map.findWithDefault 0 player2 scores > Map.findWithDefault 0 player1 scores = player2 | otherwise = player1
...
transfer from to amount bank = if Map.notMember to bank || Map.notMember from bank || amount < 0 || Map.lookup from bank < Just amount then bank else Map.adjust (\y -> y - amount) from (Map.adjust (amount +) to bank)
...
maxIndex p = fst (foldr (\x y -> if snd x == maximum p then x else y) (head (assocs p)) (assocs p)) 

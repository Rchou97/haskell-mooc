...
merge xs [] = xs
merge [] xs2 = xs2 
merge (x:xs) (x2:xs2) = if x > x2 then x2:merge(x:xs) xs2 else x:merge xs (x2:xs2)
...
maybeMap f [] = [] 
maybeMap f (x:xs) = matchMaybe (f x)
    where matchMaybe (Just b) = b:maybeMap f xs
          matchMaybe Nothing = maybeMap f xs